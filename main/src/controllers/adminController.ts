...
export default {
    ...
    displayUsers: async (req: Request, res: Response) => {
        try {
            ...
            console.log('DEBUG ADMINCONT cookies', req.cookies);
            console.log('DEBUG ADMINCONT token', token);
            console.log('DEBUG ADMINCONT decodedToken', decodedToken);
            console.log('DEBUG ADMINCONT userRole', userRole);

            if (authResponse.data.accessGranted) {
                const response = await axios.get(`${apiUsersUrl}`);
                const users = response.data;
                console.log('DEBUG ADMINCONT users', users);
                res.render('users', { users });
            } else {
                // res.status(403).render('users', { error: 'Accès refusé', users: [] });
